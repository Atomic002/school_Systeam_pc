...
class DashboardController extends GetxController {
  ...
  var ownerFundBalance = 0.0.obs; // <--- YANGI QO'SHILDI
  ...
  Future<void> _initializeData() async {
    try {
      ...
      Get.snackbar(
        'Xato',
        'Ma\'lumotlar yuklanishda xatolik yuz berdi: $e',
  ...
  Future<void> refreshData() async {
    if (isRefreshing.value || selectedBranchId.value.isEmpty) return;
    ...
    try {
      ...
      // Muvaffaqiyatli xabarni faqat qo'lda yangilanganda chiqaramiz
  ...
  Future<void> _loadAllData() async {
    await Future.wait([
      ...
      _loadCashRegisterBalance(), // Kassa balansi
    ]);
  }
  ...
  // --- 2. O'QUVCHILAR VA QARZLAR (FIXED) ---
  Future<void> _loadStudentsMetrics() async {
    try {
      ...
      activeStudents.value = studentsResponse
      ...
      // --- QARZLARNI TO'G'RI OLISH (student_debts jadvalidan) ---
      final debtsResponse = await _supabaseService.client
          .from('student_debts') // Jadval nomi to'g'irlandi
          .select('''
            student_id,
            remaining_amount,
            period_month,
          ...
          ''')
          .eq('branch_id', selectedBranchId.value)
          .eq('is_settled', false) // To'lanmagan qarzlar
          .gt('remaining_amount', 0);

      // Qarzlarini student bo'yicha guruhlash
      final Map<String, Map<String, dynamic>> debtMap = {};

      for (var item in debtsResponse) {
        final studentId = item['student_id'];
        final debt = ((item['remaining_amount'] ?? 0) as num).toDouble();
        ...
        if (debtMap.containsKey(studentId)) {
          ...
          debtMap[studentId]!['months_count'] =
              (debtMap[studentId]!['months_count'] as int) + 1;
        } else {
          debtMap[studentId] = {
            'student_id': studentId,
            'student_name': '${student['first_name']} ${student['last_name']}',
            'class_name': student['class_name'] ?? 'Sinf yo\'q',
            'phone': student['phone'] ?? '',
    ...
    try {
      final classesResponse = await _supabaseService.client
          ...
          .select('''
            ...
            room_id,
            main_teacher_id,
            rooms!classes_room_id_fkey(name),
            staff!classes_main_teacher_id_fkey(first_name, last_name, phone)
          ''')
          .eq('branch_id', selectedBranchId.value)
          .eq('is_active', true);
      ...
      for (var cls in classesResponse) {
        ...
        // Sinfning umumiy kutilayotgan tushumi
        double totalClassFee = studentsResponse.fold(
          0.0,
          (sum, s) =>
              sum +
              ((s['monthly_fee'] ?? cls['monthly_fee'] ?? 0) as num).toDouble(),
        );
        ...
        details.add({
          ...
          // Qo'shimcha dummy data (real data uchun alohida query kerak bo'ladi)
          'paid_count': 0,
          'unpaid_count': 0,
          'payment_percent': 0,
          'debt_amount': 0.0,
  ...
  Future<void> _loadStaffMetrics() async {
    try {
      ...
      teachers.value = staffResponse
          .where((s) => s['is_teacher'] == true)
          .length;
      ...
      final unpaidSalariesResponse = await _supabaseService.client
          ...
          ) // Staff orqali filialni tekshirish
  ...
  Future<void> _loadRecentActivities() async {
    try {
      // Students jadvalida class_name borligi uchun joinni soddalashtiramiz
      final paymentsResponse = await _supabaseService.client
          ...
          .select('''
            ...
            payment_time,
            payment_method,
            students!inner(first_name, last_name, class_name)
          ''')
          .eq('branch_id', selectedBranchId.value)
          .eq('payment_status', 'paid')
  ...
  Future<void> _checkSubscription() async {
    try {
      ...
      if (response != null && response['setting_value'] != null) {
        ...
        isSubscriptionExpiring.value =
            daysUntilExpiry.value <= 30 && daysUntilExpiry.value >= 0;
      }
    } catch (e) {
  ...
  Future<void> markAllNotificationsAsRead() async {
    try {
      ...
      print('Error marking all notifications: $e');
    }
  }
